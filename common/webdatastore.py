...
class webdatastore(object):
    ...
    def set_permissions(self, path):
        if os.path.exists(path):
            cmd0 = 'chmod -R 777 %s' %str(path)
            cmd1 = 'chown -R apache:apache %s' %str(path)
            #os.system(cmd0)
            os.system(cmd1)
        return
    ...
    def __read_from_shelve(self, fname, default=None):
        if os.path.isfile(fname):
    ...
    def __write_to_lmdb(self, fname, idata, wkeylst, nflag=0):
        ...
        if 1 and os.path.exists(fname):
            self.set_permissions(fname)
    ...
    def __remove_from_lmdb_keys(self, fname, rkeys, default={}):
        retval = 0
